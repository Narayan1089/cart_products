        ...
        trailing: IconButton(
          ...
          onPressed: () {
            ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Product remove from the cart.')));
          ...
          children: [
            Consumer<CartProvider>(
                builder: (context, cart, child) =>
                    Text(cart.getTotalAmount().toString(), style: hugeStyle)),
            const SizedBox(width: 24),
            TextButton(
              onPressed: () {
                ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Order not supported yet.')));
              },
